...
function StatisticsDatePicker({ fromDate, toDate, setFromDate, setToDate }: {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <div className="flex flex-col gap-1">
        <Label className="text-xs text-muted-foreground">{t('admin.to')}</Label>
        <Popover>
          <PopoverTrigger asChild>
            <Button variant="outline" className={cn("w-[140px] justify-start text-left font-normal h-9 text-sm")}>
              <CalendarIcon className="mr-2 h-4 w-4" />
...
function ReportsPanel({ admin, canSeeUser }: { admin: any; canSeeUser: (id: string) => boolean }) {
  const { t } = useTranslation();
  const dateLocale = useDateLocale();
  ...
  const timeEntries = useMemo(() => (admin.allTimeEntries?.data ?? []).filter((te: any) => {
    if (!canSeeUser(te.user_id)) return false;
    const d = format(new Date(te.start_time), 'yyyy-MM-dd');
    return d >= fromStr && d <= toStr;
  }), [admin.allTimeEntries?.data, fromStr, toStr]);

  const projectHours = useMemo(() => (admin.projectHours?.data ?? []).filter((ph: any) => {
    if (!canSeeUser(ph.user_id)) return false;
    return ph.date >= fromStr && ph.date <= toStr;
  }), [admin.projectHours?.data, fromStr, toStr]);

  const travelExpenses = useMemo(() => (admin.allTravelExpenses?.data ?? []).filter((te: any) => {
    if (!canSeeUser(te.user_id)) return false;
    return te.date >= fromStr && te.date <= toStr;
  }), [admin.allTravelExpenses?.data, fromStr, toStr]);

  const absences = useMemo(() => (admin.absences?.data ?? []).filter((a: any) => {
    if (!canSeeUser(a.user_id)) return false;
    return a.start_date <= toStr && a.end_date >= fromStr;
  }), [admin.absences?.data, fromStr, toStr]);

  const vacationRequests = useMemo(() => (admin.vacationRequests?.data ?? []).filter((v: any) => {
    if (!canSeeUser(v.user_id)) return false;
    return v.start_date <= toStr && v.end_date >= fromStr;
  }), [admin.vacationRequests?.data, fromStr, toStr]);

  const loginSessions = useMemo(() => (admin.loginSessions?.data ?? []).filter((ls: any) => {
    if (!canSeeUser(ls.user_id)) return false;
    const d = ls.login_at?.slice(0, 10) ?? '';
    return d >= fromStr && d <= toStr;
  }), [admin.loginSessions?.data, fromStr, toStr]);

  const exportWorkingHoursCSV = () => {
    const projects = admin.projects?.data ?? [];
    ...
    const rows = timeEntries.map((te: any) => {
      ...
      return [
        nameMap[te.user_id] ?? te.user_id,
        format(start, 'dd.MM.yyyy'),
        format(start, 'HH:mm'),
        end ? format(end, 'HH:mm') : '',
        te.break_minutes ?? 0,
        (Math.max(0, mins) / 60).toFixed(1),
        projectMap[te.project_id] ?? '',
        te.status,
      ].join(',');
    });
    downloadCSV([headers.join(','), ...rows].join('\n'), `working-hours-${fromStr}-${toStr}.csv`);
  };

  const exportProjectHoursCSV = () => {
    const projects = admin.projects?.data ?? [];
    const projectMap: Record<string, string> = {};
    projects.forEach((p: any) => { projectMap[p.id] = p.name; });

    const headers = ['Employee', 'Date', 'Project', 'Hours', 'Description', 'Status'];
    const rows = projectHours.map((ph: any) => [
      nameMap[ph.user_id] ?? ph.user_id,
      ph.date,
      projectMap[ph.project_id] ?? '',
      ph.hours,
      `"${(ph.description ?? '').replace(/"/g, '""')}"`,
      ph.status,
    ].join(','));
    downloadCSV([headers.join(','), ...rows].join('\n'), `project-hours-${fromStr}-${toStr}.csv`);
  };

  const exportTravelExpensesCSV = () => {
    const headers = ['Employee', 'Date', 'Kilometers', 'Parking (€)', 'Description', 'Status'];
    const rows = travelExpenses.map((te: any) => [
      nameMap[te.user_id] ?? te.user_id,
      te.date,
      te.kilometers ?? 0,
      te.parking_cost ?? 0,
    ...
    downloadCSV([headers.join(','), ...rows].join('\n'), `travel-expenses-${fromStr}-${toStr}.csv`);
  };

  const exportAbsencesCSV = () => {
    const headers = ['Employee', 'Type', 'Start Date', 'End Date', 'Status'];
    const rows = absences.map((a: any) => [
      nameMap[a.user_id] ?? a.user_id,
      a.type,
      a.start_date,
      a.end_date,
      a.status,
    ].join(','));
    downloadCSV([headers.join(','), ...rows].join('\n'), `absences-${fromStr}-${toStr}.csv`);
  };

  const exportVacationsCSV = () => {
    const headers = ['Employee', 'Start Date', 'End Date', 'Status', 'Comment'];
    const rows = vacationRequests.map((v: any) => [
      nameMap[v.user_id] ?? v.user_id,
      v.start_date,
      v.end_date,
      v.status,
      `"${(v.comment ?? '').replace(/"/g, '""')}"`,
    ].join(','));
    downloadCSV([headers.join(','), ...rows].join('\n'), `vacations-${fromStr}-${toStr}.csv`);
  };

  const exportLoginSessionsCSV = () => {
    const headers = ['Employee', 'Login Time', 'Logout Time', 'Login Lat', 'Login Lng', 'Logout Lat', 'Logout Lng'];
    const rows = loginSessions.map((ls: any) => [
      nameMap[ls.user_id] ?? ls.user_id,
      ls.login_at ? format(new Date(ls.login_at), 'dd.MM.yyyy HH:mm') : '',
      ls.logout_at ? format(new Date(ls.logout_at), 'dd.MM.yyyy HH:mm') : '',
      ls.login_lat ?? '',
      ls.login_lng ?? '',
      ls.logout_lat ?? '',
      ls.logout_lng ?? '',
    ].join(','));
    downloadCSV([headers.join(','), ...rows].join('\n'), `login-sessions-${fromStr}-${toStr}.csv`);
  };
  ...
  const reports = [
    { label: t('admin.workingHours'), count: timeEntries.length, onExport: exportWorkingHoursCSV },
    { label: t('admin.projectHoursTitle'), count: projectHours.length, onExport: exportProjectHoursCSV },
    { label: t('admin.travelExpensesTitle'), count: travelExpenses.length, onExport: exportTravelExpensesCSV },
    { label: t('admin.absences'), count: absences.length, onExport: exportAbsencesCSV },
    { label: t('admin.vacationApprovals'), count: vacationRequests.length, onExport: exportVacationsCSV },
    { label: t('admin.loginSessions'), count: loginSessions.length, onExport: exportLoginSessionsCSV },
  ];

  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-display font-bold">{t('admin.reports')}</h2>
        <p className="text-sm text-muted-foreground">{t('admin.reportsDesc')}</p>
      </div>
          ...
          <Card key={report.label}>
            <CardContent className="p-4 flex flex-col gap-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground">{report.count} {t('admin.entries')}</p>
              </div>
              <Button variant="outline" size="sm" className="gap-1.5 text-xs w-full" onClick={report.onExport} disabled={report.count === 0}>
                <Download className="h-3.5 w-3.5" /> CSV
              </Button>
            </CardContent>
          </Card>
